...
class PyPICommandError(SyncToolsError):
    """A pip subprocess call returned non-zero or timed out."""
    ...
    def __init__(self, cmd: list[str], returncode: int, stderr: str) -> None:
        ...
        self.stderr = stderr
        super().__init__(
            f"pip command failed (exit {returncode}): {' '.join(cmd)}\n{stderr}".strip()
        )


class PyPINotInstalledError(SyncToolsError):
